...
<Note>
Get your API key from the [Dashboard](https://facilitator.agentokratia.com). Keys are prefixed `ak_test_` (testnet) or `ak_live_` (mainnet).
</Note>
...
The SDK handles all API calls automatically. See [Client Integration](/integration/client) and [Server Integration](/integration/server).
