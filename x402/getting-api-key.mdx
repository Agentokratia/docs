...
<Note>
  The same facilitator URL handles both Base Mainnet and Base Sepolia. API keys are **wallet-scoped** (not network-specific) - you use the same key for all networks. Sessions are created per-network.
</Note>
...
| Scope | Description |
|-------|-------------|
| **Wallet** | API keys are tied to your wallet address |
| **All Networks** | Same key works on Base Mainnet and Base Sepolia |
